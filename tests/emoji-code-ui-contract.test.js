...
assert.match(html, /id="emojiCodeChangeMovieButton"/);
assert.match(html, /id="emojiCodeGuessSuggestions"[^>]*role="listbox"/);
assert.doesNotMatch(html, /id="emojiCode(?:Leader)?CodeInput"[^>]*placeholder="[^"]*[\p{Extended_Pictographic}]/u);
assert.match(html, /id="emojiCodeShareButton"[\s\S]*id="emojiCodeRestartButton"[\s\S]*id="emojiCodeLeaveButton"/);
assert.match(html, /\/emoji-code\.css/);
assert.match(html, /\/emoji-code\.js/);
assert.match(styles, /@media \(max-width: 760px\)/);
assert.match(styles, /grid-template-columns:\s*minmax\(76px, 24\.5vw\)\s+minmax\(0, 1fr\)/);
...
assert.match(script, /emoji-code-result-attempts/);
assert.match(script, /result\.attempts >= guessLimit/);
assert.match(script, /reroll-movie/);
assert.match(script, /\/movies\?/);
assert.doesNotMatch(script, /emoji-code-avatar/);
assert.match(script, /card\.style\.backgroundColor = item\.color/);
assert.match(script, /emojiCodeRestartButton"\)\.hidden = !isHost \|\| isLobby/);
