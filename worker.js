...
export default {
  async fetch(request, env) {
    try {
      ...
      if (request.method === "POST" && multiplayerKickMatch) return kickMultiplayerPlayer(request, multiplayerKickMatch[1], env);
      ...
      if (request.method === "POST" && impostorAdvanceMatch) return advanceimpostorRound(request, impostorAdvanceMatch[1], env);
      ...
      if (request.method === "POST" && masterWordKickMatch) return kickMasterWordPlayer(request, masterWordKickMatch[1], env);
      ...
      if (request.method === "POST" && scoreboardActionMatch) return handleScoreboardAction(request, scoreboardActionMatch[1], scoreboardActionMatch[2], env);
...
async function kickMultiplayerPlayer(request, roomName, env) {
  const room = await loadMultiplayerRoom(normalizeRoomKey(roomName), env);
  if (!room) return json({ error: "Room not found" }, 404);
  const body = await request.json().catch(() => ({}));
  const player = authenticateMultiplayerPlayer(room, body);
  const hostId = getMultiplayerHostId(room);
  if (!player || player.id !== hostId) return json({ error: "Only room creator can kick players" }, 403);
...
function mergeMultiplayerRooms(latest, incoming) {
  ...
  const mergedPlayers = [...players.values()].filter((player) => !removedIds.has(player.id));
  const merged = {
    ...latest,
    ...incoming,
...
async function listScoreboardRooms(env) {
  ...
  const result = await env.LEADERBOARD_DB.prepare(
    "SELECT state_json AS stateJson FROM multiplayer_rooms WHERE room_key LIKE 'scoreboard:%'"
  ).all();
  const rooms = (result.results || [])
    .map((row) => {
      try { return JSON.parse(row.stateJson); } catch { return null; }
    })
    .filter((room) => room && isScoreboardHostConnected(room))
...
async function createScoreboardRoom(request, env) {
  if (!env.LEADERBOARD_DB) return json({ error: "Scoreboard storage is not configured" }, 503);
  const room = normalizeNewScoreboardRoom(await request.json().catch(() => ({})));
  if (!room) return json({ error: "Invalid room, administrator, or scoreboard settings" }, 400);
  room.passwordHash = room.password ? await hashScoreboardPassword(room.password) : "";
  delete room.password;
  const existing = await loadScoreboardRoom(room.key, env);
  if (existing && isScoreboardHostConnected(existing)) return json({ error: "Room name is already in use" }, 409);
  if (existing) await env.LEADERBOARD_DB.prepare("DELETE FROM multiplayer_rooms WHERE room_key = ?").bind(scoreboardStorageKey(room.key)).run();
  await saveScoreboardRoom(room, env, true);
  return json(scoreboardRoomResponse(room, true), 201);
}

async function getScoreboardRoom(roomName, request, env) {
  const room = await loadScoreboardRoom(normalizeScoreboardRoomKey(roomName), env);
  if (!room) return json({ error: "Room not found" }, 404);
  const url = new URL(request.url);
  if (authenticateScoreboardHost(room, { hostId: url.searchParams.get("hostId"), token: url.searchParams.get("token") })) {
...
async function handleScoreboardAction(request, roomName, action, env) {
  const key = normalizeScoreboardRoomKey(roomName);
  const room = await loadScoreboardRoom(key, env);
  if (!room) return json({ error: "Room not found" }, 404);
  const body = await request.json().catch(() => ({}));
  if (action === "join") return joinScoreboardRoom(room, body, env);
  if (action === "log") return json({ events: [...getScoreboardActivityLog(room)].sort((a, b) => Number(a.createdAt || 0) - Number(b.createdAt || 0)) });
  if (room.status === "completed" && action !== "leave") {
    return json({ error: "Scoreboard session is already complete" }, 409);
  }
  if (action === "player-cell") {
    ...
    if (!room.playersCanEdit) return json({ error: "Player score editing is disabled" }, 403);
    if (room.status !== "active") return json({ error: "Current game is not active" }, 409);
  ...
  if (action === "player-role") {
    ...
    if (normalizeScoreboardGameType(room.gameType) !== "traitors-aboard") return json({ error: "Room game does not support roles" }, 409);
    if (!room.playersCanEdit) return json({ error: "Player role editing is disabled" }, 403);
    if (room.status !== "active") return json({ error: "Current game is not active" }, 409);
    if (!player || roundIndex < 0 || (body.role && !role)) return json({ error: "Invalid player role" }, 400);
    if (roundIndex !== getScoreboardCurrentRound(room)) return json({ error: "Players can only edit the current round" }, 403);
  ...
  if (!authenticateScoreboardHost(room, body)) return json({ error: "Only room administrator can edit scoreboard" }, 403);
  room.lastHostSeen = Date.now();
  ...
  if (action === "players") {
    ...
    if (!players) return json({ error: "Player names must be unique and valid" }, 400);
    ...
    if (!player || !name || !scores || duplicate) return json({ error: "Invalid or duplicate player" }, 400);
    ...
    room.players = room.players.filter((item) => item.id !== targetId);
    if (!targetId || !removedPlayer) return json({ error: "Player not found" }, 404);
    ...
    if (room.roundCount >= 80) return json({ error: "Maximum round count reached" }, 409);
    room.roundRoles = normalizeScoreboardRoundRoles(room.roundRoles, room.roundCount, new Set(room.players.map((player) => player.id)));
    room.roundCount += 1;
    ...
    if (room.roundCount <= 1) return json({ error: "Scoreboard must keep at least one round" }, 409);
    if (roundIndex < 0) return json({ error: "Invalid round" }, 400);
    room.roundRoles = normalizeScoreboardRoundRoles(room.roundRoles, room.roundCount, new Set(room.players.map((player) => player.id)));
    ...
    if (roundIndex < 0 || (hasAmount && amount === undefined) || (hasPlayerUpdate && !playerIds.size) || (!hasPlayerUpdate && !hasWinner)) return json({ error: "Invalid round update" }, 400);
    if ((hasRole || hasWinner) && normalizeScoreboardGameType(room.gameType) !== "traitors-aboard") return json({ error: "Room game does not support roles" }, 409);
    if ((hasRole && !role) || (body.winner && !winner)) return json({ error: "Invalid round roles" }, 400);
    ...
    if (hasPlayerUpdate && !selectedPlayers.length) return json({ error: "No selected players found" }, 404);
    ...
    const winner = normalizeScoreboardWinner(body.winner);
    if (normalizeScoreboardGameType(room.gameType) !== "traitors-aboard") return json({ error: "Room game does not support roles" }, 409);
    if (roundIndex < 0 || !Array.isArray(body.assignments) || (body.winner && !winner)) return json({ error: "Invalid round roles" }, 400);
    const playersById = new Map(room.players.map((player) => [player.id, player]));
    const roles = {};
    for (const assignment of body.assignments) {
      const playerId = String(assignment?.playerId || "");
      const role = normalizeScoreboardRole(assignment?.role);
      if (!playersById.has(playerId) || !role) return json({ error: "Invalid round role assignment" }, 400);
      roles[playerId] = role;
    }
    room.roundRoles = normalizeScoreboardRoundRoles(room.roundRoles, room.roundCount, new Set(playersById.keys()));
    const previous = room.roundRoles[roundIndex];
    ...
    if (!direction || nextRound < 0 || nextRound >= room.roundCount) return json({ error: "Round limit reached" }, 409);
    ...
    if (!sortOrder) return json({ error: "Invalid score order" }, 400);
    ...
    if (!room.players.length) return json({ error: "Add at least one player before finishing" }, 409);
    ...
    if (!room.players.length) return json({ error: "Add at least one player before finishing" }, 409);
    if (!Array.isArray(room.games) || !room.games.length) return json({ error: "No previous games to aggregate" }, 409);
    addScoreboardScoreEvent(room, room.hostId, room.hostName, fillScoreboardBlankScores(room));
    room.status = "completed";
    room.resultScope = "all";
    room.resultId = crypto.randomUUID();
    addScoreboardLogEvent(room, { type: "session-finished", actorId: room.hostId, actorName: room.hostName });
    room.updatedAt = Date.now();
  } else if (action === "new-game") {
    if (room.status !== "finished") return json({ error: "Finish current game before starting another" }, 409);
    ...
    await env.LEADERBOARD_DB.prepare("DELETE FROM multiplayer_rooms WHERE room_key = ?").bind(scoreboardStorageKey(key)).run();
    return json({ ok: true });
  }
...
async function joinScoreboardRoom(room, body, env) {
  if (body?.spectator) return json({ ...scoreboardRoomResponse(room), viewer: { role: "spectator" } });
  const nickname = normalizeScoreboardName(body?.nickname);
  if (!nickname) return json({ error: "Add a valid nickname" }, 400);
  if (room.passwordHash && await hashScoreboardPassword(body?.password) !== room.passwordHash) {
    return json({ error: "Incorrect room password" }, 403);
  }
  ...
  if (room.status === "completed") {
    return json({ ...scoreboardRoomResponse(room), viewer: { role: "spectator", sessionComplete: true } });
  }
  if (room.players.length >= 60) return json({ error: "Room player limit reached" }, 409);
...
async function loadScoreboardRoom(key, env) {
  if (!key || !env.LEADERBOARD_DB) return null;
  const row = await env.LEADERBOARD_DB.prepare("SELECT state_json AS stateJson, updated_at AS updatedAt FROM multiplayer_rooms WHERE room_key = ?")
    .bind(scoreboardStorageKey(key)).first();
  return row?.stateJson ? { ...JSON.parse(row.stateJson), _version: row.updatedAt } : null;
}

async function saveScoreboardRoom(room, env, create = false) {
  const storageKey = scoreboardStorageKey(room.key);
  if (create) {
    const version = `${Date.now()}-${crypto.randomUUID()}`;
    await env.LEADERBOARD_DB.prepare("INSERT INTO multiplayer_rooms (room_key, state_json, updated_at) VALUES (?, ?, ?)")
      .bind(storageKey, scoreboardRoomJson(room), version).run();
    room._version = version;
    return;
  }
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    const result = await env.LEADERBOARD_DB.prepare(
      "UPDATE multiplayer_rooms SET state_json = ?, updated_at = ? WHERE room_key = ? AND updated_at = ?"
    ).bind(scoreboardRoomJson(room), version, storageKey, room._version || "").run();
    if (result.meta?.changes) {
      room._version = version;
      delete room._touchOnly;
      return;
    }
    const latest = await loadScoreboardRoom(room.key, env);
    if (!latest) throw new Error("Room not found");
    if (room._touchOnly) {
      Object.assign(room, latest);
      return;
    }
    room._version = latest._version;
    room.lastHostSeen = Math.max(Number(room.lastHostSeen || 0), Number(latest.lastHostSeen || 0));
  }
  throw new Error("Room update conflict");
}

function scoreboardStorageKey(key) {
  return `scoreboard:${key}`;
}

function scoreboardRoomJson(room) {
  const stored = { ...room };
  delete stored._version;
...
function normalizeNewScoreboardRoom(value) {
  ...
  return {
    ...
    password: String(value?.password || "").slice(0, 40),
    passwordHash: "",
    sortOrder,
    gameType,
...
async function hashScoreboardPassword(value) {
...
async function advanceimpostorRound(request, roomName, env) {
  const room = await loadimpostorRoom(normalizeRoomKey(roomName), env);
  if (!room) return json({ error: "Room not found" }, 404);
  const body = await request.json().catch(() => ({}));
  const player = authenticateMultiplayerPlayer(room, body);
  if (!isImpostorHost(room, player)) return json({ error: "Only host can advance" }, 403);
  if (room.status !== "round-result") return json({ error: "No round result to advance" }, 409);
...
async function kickMasterWordPlayer(request, roomName, env) {
  ...
  const player = authenticateMultiplayerPlayer(room, body);
  const hostId = room.hostId || room.players[0]?.id;
  if (!isMasterWordHost(room, player)) return json({ error: "Only host can kick players" }, 403);
  if (room.status !== "lobby") return json({ error: "Players can only be kicked before the game starts" }, 409);
  const targetId = String(body.targetPlayerId || "");
  if (!targetId || targetId === hostId) return json({ error: "Invalid player to kick" }, 400);
  if (room.players.find((item) => item.id === targetId)?.isTestPlayer) return json({ error: "Test bots cannot be kicked" }, 409);
...
function mergeMasterWordRooms(latest, incoming) {
  ...
  if (incoming._touchOnly) {
    const merged = { ...latest };
    merged.players = mergeMasterWordPlayers(latest.players, incoming.players).filter((player) => !removedIds.has(player.id));
    merged._version = latest._version;
    return merged;
  }
